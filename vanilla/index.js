...
async function handleSelectedComment(event) {
  ...
  // fetching post and user details
  const selectedCommentDetails = await getCommentDetails(commentDetails)
  renderCommentDetails(selectedCommentDetails, commentDetails)
}

// rendering comment details
const renderCommentDetails = (selectedCommentDetails, commentDetails) => {
  ...
  postTitle.textContent = `Post: ${selectedCommentDetails.post.title}`

  const comment = document.createElement('p')
  comment.textContent = `Comment: ${commentDetails.comment}`

  const postBody = document.createElement('p')
  postBody.textContent = `Post Content: ${selectedCommentDetails.post.content}`

  const userName = document.createElement('p')
  userName.textContent = `User Name: ${selectedCommentDetails.user.firstname} ${selectedCommentDetails.user.lastname}`

  const userEmail = document.createElement('p')
  userEmail.textContent = `User Email: ${selectedCommentDetails.user.email}`
